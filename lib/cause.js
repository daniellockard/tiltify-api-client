...
class Cause {
   ...
   */
  get (id, callback) {
    this._doRequest(`causes/${id}`).then((response) => {
  ...
  getTopDonors (id, timeType, callback) {
    this._sendRequest(`causes/${id}/donor_leaderboard${timeType ? '?time_type' + timeType : ''}`, callback)
  }
  ...
  getTopTeams (id, timeType, callback) {
    this._sendRequest(`causes/${id}/team_leaderboard${timeType ? '?time_type' + timeType : ''}`, callback)
  }
  ...
  getLeaderboards (id, timeType, callback) {
    this._sendRequest(`causes/${id}/user_leaderboard${timeType ? '?time_type' + timeType : ''}`, callback)
  }
}
