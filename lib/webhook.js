...
class Webhook {
  ...
  subscribe (webhookID, eventID, payload, callback) {
    // TODO: FIX TYPO WHEN TILTIFY FIXES IT
    this._doRequest(`private/webhook_enpdoints/${webhookID}/webhook_subscriptions/${eventID}`, 'PUT', payload).then(function (response) {
      callback(response.data.data)
    })
  }
